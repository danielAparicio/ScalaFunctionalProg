...
trait GameDef {
   ...
   *   x axis
   */
  //CASE CLASS inside a TRAIT
  case class Pos(x: Int, y: Int) {
    /** The position obtained by changing the `x` coordinate by `d` */
    //IMP : copy method only in CASE CLASS: http://alvinalexander.com/scala/scala-case-class-copy-method-example
    //This dx method return a copy of this Pos object with a new x value 
    //note that in this case we use a NAMED PARAMMETER only for x, so y remains as the original copied object
   ...
   */
  //IMP: sealed abstract class inside a trait
  //The difference (sealed abstract and abstract) is that all subclasses of a sealed class 
  //(whether it's abstract or not) must be in the same file as the sealed class
  //http://stackoverflow.com/questions/3032771/scalas-sealed-abstract
  sealed abstract class Move
  //we also can have case object as well as case class
   ...
   * the game.
   */
  //the start block is a block with both squares in the same Start Position
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = 
       ...
       //List() :+ (this.right,Right) :+ (this.left,Left) :+ (this.up,Up) :+ (this.down,Down)
       //IMP: for sure that there is some way to iterate over the subclasses of Move
       //With the method above we create a list every time we :+ append as is a immutable list
       List((this.right,Right),(this.left,Left),(this.up,Up),(this.down,Down))
    /**
    ...
    def legalNeighbors: List[(Block, Move)] = 
      	//we get the list with all the movements and filter it getting only the legal ones
    	this.neighbors filter (x => x._1.isLegal)
    /**
     * Returns `true` if the block is standing.
     */
    def isStanding: Boolean = b1.x == b2.x && b1.y == b2.y
    //Is standing if both squares has the same x and y position
    /**
     * Returns `true` if the block is entirely inside the terrain.
     */
    def isLegal: Boolean = terrain(b1) && terrain.apply(b2)
    //Is legal if both squares are in the terrain
    //IMPORTANT ABOUT CUSTOM TYPES: The function apply of Terrain is to get a boolean based on a position, we haven't defined APPLY
    //also can be called like terrain(b1)
  }
}
